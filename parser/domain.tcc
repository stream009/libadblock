...
struct Domain<Iterator>::Impl
{
    ...
    qi::rule<Iterator, std::string()> subdomain,
        label, label_body, let_dig, let_dig_hyp;
};

template<typename Iterator>
inline Domain<Iterator>::
Domain()
...
template<typename Iterator>
inline Domain<Iterator>::~Domain() = default;
