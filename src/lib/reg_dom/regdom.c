...
static char *
getRegisteredDomainDropI(const char *hostname, const size_t length, 
                         const tldnode *tree, int drop_unknown)
{
    // Eliminate some special (always-fail) cases first.
    if (hostname[0] == '.' || length == 0)
        return 0;
    ...
    const char *seg_end = hostname + length;
...
char *
getRegisteredDomainDrop(const char *hostname, 
                        const void *tree, int drop_unknown)
{
    return getRegisteredDomainDropI(hostname, strlen(hostname), 
                                    (const tldnode *) tree, drop_unknown);
}
...
{
    return getRegisteredDomainDropI(hostname, strlen(hostname), 
                                              (const tldnode *) tree, 0);
}

char *
getRegisteredDomain_(const char *hostname, 
                    const size_t length, const void *tree)
{
    return getRegisteredDomainDropI(hostname, 
                                    length, (const tldnode *) tree, 0);
}
